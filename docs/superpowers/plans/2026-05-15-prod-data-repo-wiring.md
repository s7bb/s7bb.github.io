...
### Task 5b: Harden the compose guard (code-review fixes)

Code review of Task 5 found three real soundness gaps:

1. **False-PASS hole:** `^\s*-\s*\.:/repo\s*$` misses `.:/repo:ro`,
   `.:/repo:rw`, quoted, and `./:/repo` — the bug could return in a
   trivially-equivalent paraphrase while the guard stays green.
2. **`_service_block` silent truncation:** a future 2-space-indented
   comment/anchor ends the block early → `test_fetcher_does_not_mount
   _the_code_repo` passes vacuously (the dangerous direction).
3. **CRLF / formatting fragility** in
   `test_repo_init_service_and_volume_declared` (`split("\nvolumes:\n")`
   and `"\n  s7bb-repo-init:\n"` break on CRLF / inline comments).

Replace the entire contents of `fetcher/tests/test_compose_wiring.py`
with the hardened version below (broadened defect regex with documented
short-form scope; comment-aware block terminator; CRLF-normalised
fixture; regex-based volume/init checks; tightened `depends_on`
...
- [ ] **Step 2: Prove both reds (historical bare + paraphrase)**
...
old = open('/tmp/old-compose.yml').read()
blk = m._service_block(old, 's7bb-fetcher')
assert m._CODE_REPO_AT_REPO.search(blk), 'broadened regex must still catch bare .:/repo'
assert '\n  s7bb-repo-init:\n' not in old
# paraphrase the reviewer flagged: :ro variant must also be caught
snippet = '''services:
  s7bb-fetcher:
    volumes:
      - .:/repo:ro
  s7bb-dev:
    image: x
'''
assert m._CODE_REPO_AT_REPO.search(m._service_block(snippet, 's7bb-fetcher')), \
    'guard must catch .:/repo:ro paraphrase'
print('reds confirmed: bare + :ro paraphrase both caught')
"
rm -f /tmp/old-compose.yml
```

Expected: `reds confirmed: bare + :ro paraphrase both caught`. If not, STOP — do not commit.
...
```bash
git add fetcher/tests/test_compose_wiring.py
git -c user.name="Martin Zehetmayer" -c user.email="angrox@idle.at" commit -m "test: harden compose-wiring guard (code-review fixes)

Broaden the defect match to .:/repo plus :ro/:rw/quoted/./ paraphrases
(short-form bind; long-form type:bind documented out of scope). Make
_service_block comment-aware so a 2-space comment can't silently
truncate the block (vacuous pass). CRLF-normalise the fixture and use
regex for the init/volume checks so editor/OS reflow doesn't false-fail."
```
