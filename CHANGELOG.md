...
- **Site dev container:** `dev-entrypoint.sh` no longer shifts every `scheduled_time` / `actual_time` / `expected_slot` by +1 minute. The shift compensated for a stale committed sample data file; the dev pipeline now mounts the live `s7bb-data` tip via `s7bb-data-init`, so the shift double-corrected and rendered every dev departure 1 minute off the deployed site. Replaced the node transform with a verbatim `cp`.
...
- Classify Munich-direction trains by whether `München` appears
  anywhere in `dp.ppth`, not only at the terminus. During Munich
  Stammstrecke construction the S5 substitute service stitches
  S7-Süd onto S7-Nord, so a Wolfratshausen-bound Baierbrunn
  departure can list its terminus as Kreuzstraße / Aying /
  Höhenkirchen-Siegertsbrunn while still being a "to-Munich" run
  for our purposes. The previous terminus-only check bucketed
  these as `unknown` instead of `muenchen`.

## [0.8.1] - 2026-05-23

### Fixed

- Parser now accepts both `S7` and `S5` line codes for Baierbrunn
  arrivals. DB Timetables re-labels S7-Süd trains as `S5` while the
  Munich Stammstrecke is closed for construction; under the previous
  S7-only filter every Munich-bound row was silently dropped for the
  duration of the closure, producing an empty München column on the
  site.
